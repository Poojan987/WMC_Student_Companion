...
class particular_student(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for i in range(len(stud_data[0]['course'])):
            ...
            stud_data[0]['course'][i]['completedCourse'] = courseStudentSerializer(
                CourseStudent.objects.filter(student__in=stud_obj, course=stud_data[0]['course'][i]['id'])[0]).data
            totalobjData = courseStudentSerializer(CourseStudent.objects.filter(
                course=stud_data[0]['course'][i]['id'])[0]).data
            avgrating = 0
            minus = 0
            n = len(CourseStudent.objects.filter(
                course=stud_data[0]['course'][i]['id']))
            for j in range(len(CourseStudent.objects.filter(course=stud_data[0]['course'][i]['id']))):
                cor = courseStudentSerializer(CourseStudent.objects.filter(
                    course=stud_data[0]['course'][i]['id'])[j]).data
            ...
            stud_data[0]['course'][i]['faculty'] = FacultySerializer(
                Faculty.objects.get(id=stud_data[0]['course'][i]['faculty'])).data
            stud_data[0]['course'][i]['building'] = BuildingSerializer(
                Building.objects.get(id=stud_data[0]['course'][i]['building'])).data
            cat = []
            for j in range(len(stud_data[0]['course'][i]['cat'])):
                cat.append(CategorySerializer(category.objects.get(
                    id=stud_data[0]['course'][i]['cat'][j])).data)
...
class ToDoCreateList(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        request.data['student'] = Student.objects.get(
            user=NewUser.objects.get(email=request.user)).id
        # print(request.data)
        return self.create(request, *args, **kwargs)
...
class completedCourse(generics.ListCreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        student_obj = Student.objects.get(
            user=NewUser.objects.get(email=user).id)
        completedCourseStudent = CourseStudent.objects.filter(
            student=student_obj, completed=True)
        course_obj = []
        ...
        for i in range(len(completedCourseStudent)):
            ...
            dic = tp
            dic['faculty'] = FacultySerializer(
                Faculty.objects.get(id=tp['faculty'])).data
            dic['building'] = BuildingSerializer(
                Building.objects.get(id=tp['building'])).data
            # dicMain['course']=dic
...
class creditDetail(generics.ListCreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        user = request.user
        stud_obj = studentSerializer(Student.objects.get(
            user=NewUser.objects.get(email=user).id)).data
        stud_data = stud_obj
        # print(stud_data)
        program_obj = Programme.objects.get(name=stud_obj['program'])

        program_data = programeSerializer(program_obj).data
        requiredcredit_obj = CreditModel.objects.filter(
            program=program_data['id'])
...
#       Processing student completed courses
            ...
            for j in range(len(stud_data['course'])):
                ...
                courseStudent_data = courseStudentSerializer(CourseStudent.objects.filter(
                    student=stud_data['id'], course=stud_data['course'][j]['id'])[0]).data

                if(courseStudent_data['completed'] == False):
                    continue
                for k in range(len(stud_data['course'][j]['cat'])):

                    categ = CategorySerializer(category.objects.get(
                        id=stud_data['course'][j]['cat'][k])).data
                    # print(categ)
...
class RatingPost(generics.UpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        stud = request.user
        request.data['student'] = Student.objects.get(
            user=NewUser.objects.get(email=stud)).id
        return self.update(request, *args, **kwargs)
